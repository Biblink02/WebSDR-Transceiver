...
<template>
    <AppLayout>
        <div class="max-w-7xl mx-auto p-15 font-mono">
            ...
            <div v-for="resource in resources" :key="resource.id" class="flex items-center space-x-6 rounded-lg p-6">
                <a :href="resource.url" class="flex-shrink-0 w-40 h-28 overflow-hidden rounded-md border border-gray-300"
                   target="_blank">
                    <img :src="resource.thumbnail" alt="Thumbnail" class="object-cover w-full h-full"/>
                </a>
                <div class="flex-1">
                    <a :href="resource.url" class="text-2xl font-semibold hover:underline text-favourite_yellow"
                       target="_blank">
                        {{ resource.title }}
                    </a>
